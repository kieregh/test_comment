 ...
 !function(a) {
    ...
    a.fn.uploadFile = function(c) {
        ...
        function l(b) {
         ...
         function m(b, c, d) {
            for (var e = 0; e < d.length; e++) {
               ...
               // 파일 데이타 세팅 
                f.fileData = g;
                var o = new q(c, b); // 317 라인 엘리먼트 클래스 인스턴스 생성
                var p = "";
               if (b.showFileCounter) p = c.fileCounter + b.fileCounterStyle + d[e].name; else p = d[e].name; // 파일순서, 순서 형식, 이름 세팅
               var fn=d[e].name;
               var fnSp=fn.split('.');
               var fExt=fnSp[1];
                //if (b.showFileCounter) p = '<img src="'+b.saveDir+upcode+'.'+fExt+'" width="100px" height="100px"/>';
                o.statusbar.html('');
        ...
        function q(b, c) {
            ...
            this.progressDiv = a("<div class='upload-progress'>").appendTo(this.statusbar);
