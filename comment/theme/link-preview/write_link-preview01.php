...
<!-- 엔터키값으로 폼 전송되지 않도록 onkeypress="return event.keyCode != 13;" 추가함 -->
<form name="shareForm" id="shareForm" role="form" method="post" action="<?php echo $g['s']?>/"  target="_action_frame_comment" enctype="multipart/form-data" onsubmit="return false;">
        ...
        <div class="share-form">
            ...
            <div class="share">
                ...
                <div id="status-box" class="shareType-box">
                   <img id="link-preview-indicator" src="<?php echo $g['url_comment_skin']?>/img/ajax-loader.gif">
                   <textarea placeholder="지금 무슨 생각하세요?" class="form-control message" id="status_message" rows="1"  name="content"></textarea> 
                   <div id="link-preview"><!-- 링크 프리뷰 내용 출력 --> </div>
                </div>
...
$(document).ready(function() {
  ...
  var getUrl  = $('#status_message'); //url to extract from text field
  ...
  getUrl.keyup(function() { //user types url in text field    
    ...
    if (match_url.test(getUrl.val())) {
        $("#link-preview").hide();
        $("#link-preview-indicator").show(); //show loading indicator image
        
        var extracted_url = getUrl.val().match(match_url)[0]; //extracted first url from text filed
        
        //ajax request to be sent to extract-process.php
        $.post(rooturl+'/?m=comment&a=extract_url',{'url': extracted_url}, function(data){         
          ...
          if(total_images>0){
            inc_image = '<div class="media-left" id="extracted_thumb"><img src="'+data.images[img_arr_pos]+'" width="100" height="100"></div>';
          }else{
            inc_image ='';
          }
          //content to be loaded in #link-preview element
          var content = '<div class="media">'+ inc_image +'<div class="media-body"><h4 class="media-heading rb-title">';
          content +='<a href="'+extracted_url+'" target="_blank">'+data.title+'</a></h4>';
          content +='<p class="rb-description">'+data.description+'</p>';
          content +='<ul class="list-inline">';
          content +='<li><button id="thumb_prev" class="btn btn-sm"><i class="fa fa-caret-left"></i></button><button id="thumb_next" class="btn btn-sm"><i class="fa fa-caret-right"></i></button><li>';
          content +='<span id="total_imgs">'+img_arr_pos+' of '+total_images+'</span><li></ul></div></div>';
          
          //load link-preview in the element
          $("#link-preview").html(content); //append received data into the element
          $("#link-preview").slideDown(); //show link-preview with slide down effect
          $("#link-preview-indicator").hide(); //hide loading indicator image
                },'json');
    }
  });
