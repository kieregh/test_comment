...
$link_preview=$g['url_comment_skin'].'/link-preview';

// 넥스트 버튼 
if($R['comment']<=$d['comment']['recnum']) $btn_next_disabled='disabled';
else $btn_next_disabled='';
?>

<link rel="stylesheet" href="<?php echo $g['url_comment_skin']?>/style.css">
<link rel="stylesheet" type="text/css" href="<?php echo $link_preview?>/linkPreview.css" />
<script type="text/javascript" src="<?php echo $link_preview?>/linkPreview.js" ></script>
<script type="text/javascript" src="<?php echo $link_preview?>/linkPreviewRetrieve.js" ></script>
<script>
...
// 댓글 등록 함수 
function comment_Regis(textarea)
{
    ...
    var f_data=JSON.stringify(formDataObj); // 최종 json data : dataform 필드값
    ...
    var uid=f.uid.value; // 해당 댓글 uid 값 
     $.ajax({
          type: "GET",
          url: rooturl+'/?r='+raccount+'&m=comment&a=comment_regis&data_type=json&data='+encodeURI(f_data),
          async: true,  
          cache: false,
          success: function (response) {
               if(!uid) $('#cmt_total').text(new_total); // uid 가 없는 경우 즉, 신규인 경우 
                $(textarea).html(''); // 글 내용 비우기 
...
$('.cmt-post').on('click', function(){
    var textarea = $(this).parent().parent().find('.textarea'); // 댓글 박스
    comment_Regis(textarea);
});
