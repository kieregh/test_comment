...
<?php  include $g['path_module'].'comment/_function.php';?> 
<link rel="stylesheet" href="<?php echo $g['dir_comment_skin']?>style.css" >
<script type="text/javascript" src="<?php echo $g['dir_comment_skin']?>jquery.form.js"></script>   
<!--// 댓글 관련 함수,스타일, js  인클루드 -->
...
<div>
  <h4><i class="fa fa-edit hidden-xs"></i> 댓글쓰기</h4>
    ...
    <input type="hidden" name="object_id" value="<?php echo $object_id?>" /><!-- 해당 오브젝트 id  -->
    <input type="hidden" name="theme" value="<?php echo $g['dir_comment_skin']?>" /><!-- 테마값  -->
    <input type="hidden" name="p_table" value="<?php echo $CO['p_table']?>" /><!-- 부모 데이타 저장 테이블  -->
    <input type="hidden" name="c_table" value="<?php echo $CO['c_table']?>" /><!-- 댓글 저장 테이블  -->
    <input type="hidden" name="c_sort" value="<?php echo $CO['c_sort']?>" /><!-- c_sort  -->
    <input type="hidden" name="c_orderby" value="<?php echo $CO['c_orderby']?>" /><!-- sort  -->
    <input type="hidden" name="c_recnum" value="<?php echo $CO['c_recnum']?>" /><!-- 출력수  -->
...
<div  id="rb-comments">
    <div class="panel panel-default">
      ...
      <ul class="list-group" id="comment-list-box">
         <?php getCommentList($object_id,$g['dir_comment_skin'],$R['uid'],$_where,$c_recnum,$c_sort,$orderby,$cp)?>
      </ul>  
...
function get_CommentList(type)
{
   ...
   var data=theme+'^^'+parent+'^^'+sort+'^^'+recnum+'^^'+page+'^^'+orderby+'^^'+last_sort;
    // 액션 페이지로 보내기  
   var ajax=getHttprequest(rooturl+'/?r='+raccount+'&m=comment&object=<?php echo $object_id?>&a=get_CommentList&type='+type+'&data='+data,''); 
   //결과값 세팅  
   ...
   if(act=='edit'){
    ...
    $('input[name="uid"]').val(uid);
      ...
      if(confirm('정말로 '+act_name[act]+'하시겠습니까?     ')){
         ...
         // 액션 파일에 전송 및 결과값 세팅 
         if(act=='good' || act=='bad')  ajax=getHttprequest(rooturl+'/?r='+raccount+'&m=comment&object=<?php echo $object_id?>&a=comment_score&uid='+uid+'&value='+act,''); // 평가인 경우 
         else ajax=getHttprequest(rooturl+'/?r='+raccount+'&m=comment&object=<?php echo $object_id?>&a=comment_'+act+'&uid='+uid,''); 
...
$('body').on('click','.one-regis',function(){
    ...
    if(content==''){
      ...
      var ajax=getHttprequest(rooturl+'/?r='+raccount+'&m=comment&object=<?php echo $object_id?>&a=oneline_regis&post='+post+'&parent='+parent+'&content='+content+'&uid='+uid,''); // 액션 페이지로 보내기  
...
$('body').on('click','a.one-tools',function(e){
   ...
   if(act=='edit'){
    ...
    $('input[name="ouid"]').val(uid);
      ...
      if(confirm('정말로 '+act_name[act]+'하시겠습니까?     ')){
        ajax=getHttprequest(rooturl+'/?r='+raccount+'&m=comment&object=<?php echo $object_id?>&a=oneline_'+act+'&uid='+uid,''); 
         var msg=getAjaxFilterString(ajax,'RESULT'); // 결과 메세지 
         // 액션처리 후 
         if(act=='report'){
            alert(msg); 
...
{
   var ajax=getHttprequest(rooturl+'/?r='+raccount+'&m=comment&object=<?php echo $object_id?>&a=get_OnelineList&theme='+theme+'&parent='+parent,''); 
   var result=getAjaxFilterString(ajax,'RESULT');
